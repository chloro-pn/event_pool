...
class TimeEvent {
 public:
  ...
  explicit TimeEvent(std::chrono::time_point<std::chrono::steady_clock, Duration> time_point) : 
    time_point_(std::chrono::time_point_cast<std::chrono::milliseconds>(time_point)), 
    type_(Type::TIME_POINT) {
...
std::chrono::time_point<std::chrono::steady_clock, std::chrono::milliseconds> SecondsAfter(uint64_t sec) {
  return std::chrono::time_point_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() + std::chrono::seconds(sec));
}
}